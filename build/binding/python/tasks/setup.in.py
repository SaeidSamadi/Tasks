...
class pkg_config(object):
    def __init__(self):
        compile_args = "$<TARGET_PROPERTY:Tasks,COMPILE_DEFINITIONS>;$<TARGET_PROPERTY:Tasks,INTERFACE_COMPILE_DEFINITIONS>"
        self.compile_args = [ "-D" + x for x in compile_args.split(';') if len(x) ]
        self.compile_args = list(set(self.compile_args))
        include_dirs = "$<TARGET_PROPERTY:Tasks,INCLUDE_DIRECTORIES>;$<TARGET_PROPERTY:Tasks,INTERFACE_INCLUDE_DIRECTORIES>"
        ...
        library_dirs = "$<TARGET_PROPERTY:Tasks,LINK_FLAGS>"
        self.library_dirs = [ x for x in library_dirs.split(';') if len(x) ]
        self.libraries = [ re.sub("^lib", "", os.path.splitext(os.path.basename(l))[0]) for l in "$<TARGET_LINKER_FILE:Tasks>".split(";") if len(l) ]
        self.libraries = list(set(self.libraries))
        self.library_dirs += [os.path.dirname(l) for l in "$<TARGET_LINKER_FILE:Tasks>".split(';') if len(l) ]
...
def cxx_standard_cmp(lhs):
    if lhs == 98:
        return 1
    return lhs
cxx_standard = max(map(cxx_standard, "0;$<TARGET_PROPERTY:Tasks,CXX_STANDARD>".split(';')), key = cxx_standard_cmp)
if cxx_standard != 0:
